...
<Tabs>
  <Tab title="Private Trackers">
    ...
    mkbrr create "Movie.2024.1080p.BluRay.x264" -t https://your-tracker.com/announce --piece-length 24
  ...
  <Tab title="Public Trackers">
    ...
    mkbrr create "Linux.ISO" -t udp://tracker1.com -t udp://tracker2.com --private=false
    ```
    
    <Note>
      Use `--private=false` for public trackers. Specify multiple trackers by repeating the `-t` flag.
    </Note>
  </Tab>
</Tabs>
...
<Accordion title="Frequently Used Flags">
  ...
  --piece-length    # Manual piece size (power of 2)
  --source          # Source tag
...
<Accordion title="Common Issues">
  ...
  ### Invalid Piece Size
  Piece size must be a power of 2 between 16 and 27:
  ```bash
  # Valid sizes: 16(64KiB), 17(128KiB), 18(256KiB)...27(128MiB)
  mkbrr create data -t tracker.url --piece-length 16
  ```
