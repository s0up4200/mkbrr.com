...
-   `entropy`: `true` to randomize the torrent's info hash by adding a unique, random key to the info dictionary before hashing (useful for cross-seeding identical content on trackers that reject duplicate info hashes). Defaults to `false`.
...
Command-line flags take precedence over preset settings. If you use a flag that corresponds to a preset option, the flag's value will be used instead of the value defined in the preset.
<Warning>
Remember: Command-line flags always override settings defined in the selected preset.
</Warning>
