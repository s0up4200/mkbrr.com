...
4.  **Warning Trigger:** A warning is displayed during torrent creation if the pack appears suspicious, primarily when the highest episode number found (`MaxEpisode`) is greater than the number of video files detected. This often indicates missing files or inconsistent naming.
